...
void CParentView::OnPipeCREATE() 
{
	...
	hPipe=CreateNamedPipe("\\\\.\\pipe\\NamedPipe",                            //创建命名管道
	PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED,0,1,1024,1024,0,NULL);
	if(hPipe == INVALID_HANDLE_VALUE) {                   //创建命名管道失败报错
		MessageBox("创建命名管道失败!");
		return;
	}
	STARTUPINFO sui;
	...
	if (!CreateProcess("C:/Users/Administrator/Desktop/OSexp5/Child/Debug/Child.exe", NULL, NULL, NULL, TRUE, 0, NULL, NULL, &sui, &pi)) {          //创建子进程
		CloseHandle(hPipe);
		hPipe = NULL;
		MessageBox("创建子进程失败!");
		return ;
...
void CParentView::OnPipeREAD() 
{
	...
	char buf[100];
	DWORD dwRead;
	if (!ReadFile(hPipe, buf, 100, &dwRead, NULL)) {            //从命名管道读取数据
		MessageBox("读取数据失败！");
		return ;
	}
	MessageBox(buf);
